...
class JSONLoader:
    ...
    def load_insertion(self, flights):
        ...
        print(f"AVL - Root: {self.avl.getRoot().getValue() if self.avl.getRoot() else 'None'}, Depth: {self.avl.getDepth()}, Leaves: {self.avl.countLeaves()}")
        # Print BST properties
        print(f"BST - Root: {self.bst.getRoot().getValue() if self.bst.getRoot() else 'None'}, Depth: {self.bst.getDepth()}, Leaves: {self.bst.countLeaves()}")
